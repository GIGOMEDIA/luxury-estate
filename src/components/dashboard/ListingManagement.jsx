...
const ListingManagement = ({ items = [], totalActive, totalPending, totalSold }) => {
  const defaultItems = [
  ...
  const managementItems = items.length
    ? items.map((item) => ({
        imagePath: resolvePropertyImage(item),
        amount: item.amount,
        placeName: item.title,
        noOfBeds: item.beds,
        noOfBaths: item.baths,
        area: item.areaSqft?.toLocaleString('en-US'),
        slug: item.slug,
      }))
    : defaultItems
    ...
    <div className="bg-white rounded-2xl p-6 shadow-sm border border-[#EEF3FA]">
      <div className="flex items-center justify-between">
        <div>
          ...
          <p className="text-sm text-[#6B7280]">Manage {totalActive ?? 42} active high-value properties{typeof totalPending === 'number' ? `, ${totalPending} pending` : ''}{typeof totalSold === 'number' ? `, ${totalSold} sold` : ''}</p>
        </div>
